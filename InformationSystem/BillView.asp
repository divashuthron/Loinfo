...
Dim LeftMenuName : LeftMenuName = "Home / 합격자발표관리 / 고지서 설정"
Dim LeftMenuNameDetail : LeftMenuNameDetail = "고지서 설정"
Dim LogDivision				: LogDivision = "BillView"
%>
...
Dim StrURL			: StrURL = "/BillList.asp"
Dim StrViewURL		: StrViewURL = "/BillView.asp"

Dim IDX				: IDX	= fnR("IDX", 0)
Dim conut, conutName

Dim ProcessType
Dim MYear,Division0,Title,Degree,State,StateName,option1,option2,option3,option4,option5,option6,option7,option8,option9,option10 
...
SQL = SQL & vbCrLf & "	IDX,MYear,Division0,Title,Degree,State, (CASE  State "
...
$(function() {
	...
	$("#btnSave").click(function() {
		...
		if ($.setValidation($("#InputForm"))) {
			...
			// 저장
			if (confirm("고지서를 저장 하시겠습니까?")) {
				var objOpt = {"url":"","param":"","dataType":"xml","before":"","success":"$.setBill(datas)","complete":"","clear":"","reset":""};
	...
	$("#btnDegreeAdd").click(function() {
		...
		if ($.setValidation($("#InputForm"))) {
			...
			$("#ProcessType").val("BillDegreeAdd");
			$("#InputForm [name='Myear']").attr("disabled", false).trigger("chosen:updated");
			$("#InputForm [name='Division0']").attr("disabled", false).trigger("chosen:updated");

			// 저장
			if (confirm("고지서를 차수를 추가 하시겠습니까?")) {
				var objOpt = {"url":"","param":"","dataType":"xml","before":"","success":"$.setBill(datas)","complete":"","clear":"","reset":""};
				objOpt["url"] = $("#InputForm").attr("action");
				$.Ajax4Form($("#InputForm"), objOpt);
				$("#InputForm").submit();
	...
	// 저장처리결과
	$.setBill = function(datas) {
		var $objList = $(datas).find("List");	
		var strMSG;
		
		if ($objList.find("Result").text() == "Complete") {
			alert("고지서 저장이 완료 되었습니다.");

			if ($("#InputForm input[name='IDX']").val() == "0") {
				document.location.href = "/BillList.asp";
			} else {
				document.location.href = "/BillList.asp";
				//document.location.reload();
			}
		} else {
			alert("고지서 저장 중 오류가 발생하였습니다.\errorCode : "+ $objList.find("ReturnMSG").text());
			return;
		}
	}
	...
	<div class="col-lg-12">
		<div class="ibox float-e-margins">
			...
			<div class="ibox-content">
				<div>
					<form name="InputForm" id="InputForm" method="post" action="/Process/BillProc.asp">
					...
					<div class="row show-grid">
						...
						<div class="col-md-2 col-xs-2">
							<% Call SubCodeSelectBox("Division0", "모집시기", Division0, "고지서모집시기를 입력해주세요.", "", "Division0") %>
						</div>
						<div class="col-md-1 col-xs-1 grid_sub_title2">
							은행명(*)
						</div>
						<div class="col-md-2 col-xs-2">
							<% Call SubCodeSelectBox("RefundBankCode", "은행명", option1, "환불은행명을 입력해주세요.", "", "RefundBankCode") %>
						</div>						
						...
						<div class="col-md-2 col-xs-3 grid_sub_title">
							<select name="SubTitle" style="width:100%;">
								<option value="" <% If option4 = "" then response.write "selected" end if%>>구분</option>
								<option value="1" <% If option4 = 1 then response.write "selected" end if%>>예치금</option>
								<option value="0" <% If option4 = 0 then response.write "selected" end if%>>본등록</option>
							</select>
						</div>	
						<div class="col-md-1 col-xs-2 grid_sub_title">
							사용여부(*)
			...
			<div class="row show-grid grid_sub_button">
				<div class="col-md-12">
					...
					<span class="btnBasic btnTypeSave" id="btnSave"><% If ProcessType = "BillUpdate" Then %>수정<% Else %>등록<% End If %></span>
					<% If ProcessType = "BillUpdate" Then %><span class="btnBasic btnTypeAdd" id="btnDegreeAdd">차수추가</span><% End If %>
