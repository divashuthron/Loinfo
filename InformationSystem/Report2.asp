...
Dim LeftMenuCode : LeftMenuCode = "Report2"
Dim LeftMenuName : LeftMenuName = "Home / 합격자발표관리 / 레포트출력2"
Dim LeftMenuNameDetail : LeftMenuNameDetail = "레포트출력2"
%>
	...
	$("#ReportExcel").click(function() {
		if (!$.chkInputValue($("#IDX"),		"저장할 레포트를 선택해주세요.")) { return; }
			...
			if (blnChkResult) {
				...
				$ParentForm.attr("action", "ReportExcel_"+idx+".asp");
				$ParentForm.submit();
			}
	});
});
