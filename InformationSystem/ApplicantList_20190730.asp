...
SQL = SQL & vbCrLf & "	left outer join (	select b.StudentRecordRatio, a.StudentNumber, c.EXAM_NUMB as StdentRecord1, c.Cors_Code, c.Majr_cs11, c.Majr_cs12, c.Majr_cs21, c.Majr_cs22, c.Majr_cs31, c.Majr_cs32, d.EXAM_NUMB as StdentRecord2, e.EXAM_NUMB as StdentRecord3, f.EXAM_NUMB as StdentRecord4  "
SQL = SQL & vbCrLf & "						FROM ApplicationTable AS a   "
SQL = SQL & vbCrLf & "							left outer join AppraisalTable AS b   "
SQL = SQL & vbCrLf & "								on (a.Myear = b.Myear and a.SubjectCode = b.SubjectCode)   "
...
SQL = ""
SQL = SQL & vbCrLf & "SELECT "
SQL = SQL & vbCrLf & "	[년도], [모집구분], [수험번호], [학과코드], [고교과목코드], [졸업학년], [반], [번호] "
SQL = SQL & vbCrLf & "	, [전공과정_1_1], [전공과정_1_2], [전공과정_2_1], [전공과정_2_2], [전공과정_3_1], [전공과정_3_2], [비고] "
SQL = SQL & vbCrLf & "FROM ApplicationTable AS A "
SQL = SQL & vbCrLf & "	INNER JOIN [V_생기부_인적사항] AS B "
SQL = SQL & vbCrLf & "		ON a.Myear = b.[년도] and a.StudentNumber = b.[수험번호] "
SQL = SQL & vbCrLf & "WHERE b.[수험번호] = ?; "

Call objDB.sbSetArray("수험번호", adVarchar, adParamInput, 50, "2120082 ")

arrParams = objDB.fnGetArray
AryHash2 = objDB.fnExecSQLGetHashMap(SQL, arrParams)

if IsArray(AryHash2) Then
	For i = o to UBOUND(AryHash2, 1)
		RW(AryHash2(i).Item("수험번호"))
	next
End if
			...
			<div class="ibox-title">
				...
				<div class="ibox-tools">
					...
					<!--<form id="PageSizeForm" method="get">-->
						<span class="btnBasic btnTypeExcel" onclick="alert('엑셀로 업로드가 지원되지 않습니다.');return false;">생기부 넣기</span>
						<span class="btnBasic btnTypeExcel" onclick="alert('엑셀로 업로드가 지원되지 않습니다.');return false;">수능성적 넣기</span>
						<span class="btnBasic btnTypeComplete" onclick="alert('중간테이블과 연결되어 있지 않습니다.');return false;">생기부 가져오기</span>
						<span class="btnBasic btnTypeComplete" onclick="alert('중간테이블과 연결되어 있지 않습니다.');return false;">수능성적 가져오기</span>
							...
							<%
								...
								If isArray(AryHash) Then								
									...
									If AryHash(i).Item("Division1") = "X05041" Then '일반고전형
										If Not(isnull(AryHash(i).Item("Cors_Code"))) And AryHash(i).Item("Cors_Code") <> "0685" Then
											'일반고, 자율고가 아님
											DrawStandard = "Y"
											If LEN(DrawTemp) < 1 Then
												DrawTemp = "일반고전형에 해당하는 교육과정을 이수하지 않았습니다."
											Else
												DrawTemp = DrawTemp & "= 일반고전형에 해당하는 교육과정을 이수하지 않았습니다."
											End If
										ElseIf AryHash(i).Item(Semester) <> "" Then	
											'일반고, 자율고이나 선택한 학기에 직업과정 위탁교육을 이수하였음.
											DrawStandard = "Y"
											If LEN(DrawTemp) < 1 Then
												DrawTemp = "선택한 학기에 직업과정 위탁교육을 이수하였습니다."
											Else
												DrawTemp = DrawTemp & "= 선택한 학기에 직업과정 위탁교육을 이수하였습니다."
											End If
										End If
									ElseIf AryHash(i).Item("Division1") = "X05042" Then '전문(직업)과정 전형
										If AryHash(i).Item("Cors_Code") = "0685" Then									
											If AryHash(i).Item(Semester) = "" Then
												...
												DrawStandard = "Y"
												If LEN(DrawTemp) < 1 Then
													DrawTemp = "전문(직업)과정전형에 해당하는 교육과정을 이수하지 않았습니다."
												Else
													DrawTemp = DrawTemp & "= 전문(직업)과정전형에 해당하는 교육과정을 이수하지 않았습니다."
												End If
											End If
										End If
									End If
									...
									'최종완료 체크(N이 있으면, 미완료)
									If StudentRecordDataStr = "N" Or QualificationDataStr = "N" Or CSATDataStr = "N" Or InterviewerStr <> "N" Or PracticalStr <> "N" Or DrawStandard <> "N" Then
										ReslutStr = "N(미완료)"
									Else
										ReslutStr = "Y(완료)"
									End If									
