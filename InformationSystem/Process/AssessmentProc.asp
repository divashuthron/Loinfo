...
'// 사정처리 순서
'// ==============================================================================================================
'// 0.셋팅	해당 년도, 해당 모집시기 초기화
'// 1.		최종이 완료인 지원자만 뽑기
'// 2.		지원자 형태 구분
'// 3.		지원자 구분 별 공식적용하여 점수 계산
'// 4.		전형별 통합점수 생성 (가산점 + 전형별 %)
'// 5.		같은 점수 찾아 동석차 순위 생성
'// 6.		석차 생성(1순위 통합점수, 2순위 독석차 순위)
'// 7.		모집단위 별 모집인원으로 합/불여부 생성
'// 8.		불합격에 대한 예비석차 생성
'// ==============================================================================================================
...
'// 동석차 기준 펑션 구분하기
'// ==============================================================================================================
'// 1.		내림차순 (면접고사 성적 상위자, 교과 성적 평균등급 상위자, 선택한 학기 이수단위 합이 높은자, 
'//					  학생부 성적 고학년 고학기 선택자, 졸업(취득) 학점이 많은 자, 국어영역 성적 상위자, 
'//					  영어영역 성적 상위자, 수학영역 성적 상위자)
'// 2.		오름차순 (연소자)
'// ==============================================================================================================
...
Dim returnMSG
Dim InsertType
Dim StudentNumLog			: StudentNumLog = 0

'2.대상구문
Dim BasicMYear						: BasicMYear = fnRF("AssessmentMyear")
Dim BasicDivision0					: BasicDivision0 = fnRF("AssessmentDivision0")
...
Dim OldDrawRanking, TxtScore, ScoreTemp, DrawStandingTemp										'// 기존랭킹구분, 점수Str, 점수교환용temp, 동석차Temp
...
SQL = ""
SQL = SQL & vbCrLf & " Delete ChangeScoreTable "
...
'// ==============================================================================================================
	...
	If Not(isnull(AryHash)) Then
		for i = 0 to ubound(AryHash,1)
			...
			If Not(isnull(AryHash2)) Then
				UnqualifiedStandard1 = AryHash2(0).Item("UnqualifiedStandard1") '동석차 기준
			End If
			...
			If Not(isnull(AryHash2)) Then
				for j = 0 to ubound(AryHash2,1)
					DrawScore = AryHash2(j).Item("totScore")			'동석차인 점수
					totScoreCount = AryHash2(j).Item("totScoreCount")	'점수별 동석차수
					totScoreCount = totScoreCount-1						'for문을 위한 -1
...
'// ==============================================================================================================
'// 5. 같은 점수 찾아 동석차 순위 생성 끝
'// ==============================================================================================================

'// ==============================================================================================================
'// 6. 석차 생성(1순위 통합점수, 2순위 동석차 순위) 
'// ==============================================================================================================
...
Function UnqualifiedStandard_Down(Check, AryStudentNumber, Score, count)
	...
	If Check Then
		...
		For DrawRankingNum3 = 0 To count2		
			...
			'Update일 때는 UPDT입력
			arrParams = Array(_
				  Array("@DrawStanding",			adInteger,		adParamInput,		0,		ArrTxtDrawRanking(DrawRankingNum3)) _
...
Call objDB.sbSetArray("@Division0", adVarchar, adParamInput, 50, BasicDivision0)
...
InsertType = "insert"
strLogMSG = "사정관리 > " & BasicMYear & "학년도_" & AryHash(0).Item("DivisionName") & "의 " & StudentNumLog & "명을 사정처리하였습니다."
