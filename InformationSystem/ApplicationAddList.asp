...
Dim LeftMenuCode : LeftMenuCode = "ApplicationAdd"
Dim LeftMenuName : LeftMenuName = "Home / 입학원서관리 / 입학원서 수동입력"
Dim LeftMenuNameDetail : LeftMenuNameDetail = "입학원서 수동입력"
Dim LogDivision	: LogDivision = "ApplicationAddList"
%>
<!-- #include virtual="/Common/Header.asp" -->
<%
Dim objDB, objDB2, SQL, arrParams, arrParams2, aryList, AryHash, AryHash2, strWhere
...
SQL = SQL & vbCrLf & "WHERE INPT_USID = '" & SessionUserID & "' "
...
strLogMSG = "입학원서관리  > " & SessionUserID  &"가/이 입학원서 수동입력 리스트를 조회 했습니다."
...
SQL = ""
SQL = SQL & vbCrLf & "Select ApplicationAddConfirm "
...
$(function() {
	...
	$(document).delegate("tr.viewDetail_SetDate_2", "click", function() {
		...
		$("#InputForm input[name='StudentNumber']").prop("readonly", true);
		$("#InputForm [name='MYear']").prop("readonly", true).trigger("chosen:updated");
	});
	...
	$("#btnNew").click(function () {
		if (confirm("입력되어 있던 내용이 초기화 됩니다.\n신규로 입력 하시겠습니까?")) {
			$.FormReset($("#InputForm"));
			$("#InputForm input[name='StudentNumber']").prop("readonly", false);
			$("#InputForm [name='MYear']").prop("readonly", false).trigger("chosen:updated");
			$("#InputForm [name=MYear]").val(SessionMYear).prop("selected", true).trigger("chosen:updated");
			$("#InputForm [name=Division0]").val(SessionDivision0).prop("selected", true).trigger("chosen:updated");
		}
	});
...
<div class="row">
	<div class="col-lg-12">
		...
		<% 
		If AryHash2(0).Item("ApplicationAddConfirm") = "N" Then
		%>
			<div class="ibox-content" style="padding:350px;">
				<div style="text-align: center;">
					<h2>입학원서 수동입력 페이지 사용이 중단되었습니다. 관리자에게 문의하세요.</h2>
				</div>
			</div>
		<%
		Else
			...
			<div class="ibox-content">
				<form name="InputForm" id="InputForm" method="post" action="/Process/ApplicationProc.asp">
					...
					<div class="row show-grid">
						...
						<div class="col-md-2 col-xs-7 grid_sub_title">
							<% Call SubCodeSelectBox("Semester", "반영학기 선택", "", "", "", "Semester") %>
						</div>
					</div>
					<div class="row show-grid">					
						<div class="col-md-2 col-xs-2 grid_sub_title">
					...
					<div class="row show-grid">
						...
						<div class="col-md-2 col-xs-3 grid_sub_title">
							<div class="input-group viewCalendarBtn" Obj="ReceiptDate" top="885">
								<input type="text" name="ReceiptDate" id="ReceiptDate" class="form-control input-sm" maxlength="10">
						...
						<div class="col-md-2 col-xs-3">
							<div class="input-group">
								<input type="text" name="CheckTime" id="CheckTime" class="form-control input-sm" maxlength="5" data-autoclose="true">
